"""
build_notebooks.py — generates the journal notebooks from the private local
content in data/*.py:

  - notebooks/YYYY-MM.ipynb            one per documented month
  - notebooks/years/YYYY.ipynb         year-in-review rollup
  - notebooks/global_track_record.ipynb   full-history rollup
  - trades/YYYY-MM.md                  plain-markdown version of each month,
                                        readable without Jupyter

Generated notebooks are self-contained: each one embeds its own trade entries
directly as a Python literal, so it never reads data/ (or any other file
outside itself) at run time. data/*.py is only ever read here, at build time,
and stays local (see .gitignore) — it never gets published.

Usage:
    python3 src/build_notebooks.py            # (re)generate everything
    python3 src/build_notebooks.py --execute   # + run each notebook (nbconvert)
                                                #   so charts render statically
                                                #   on GitHub without the reader
                                                #   having to execute anything
    python3 src/build_notebooks.py --only 2024-06   # only this month
"""
...
def entry_cell_markdown(e: dict) -> str:
    return f"### {e['title']}\n\n*{meta_line(e)}*\n\n{e['body']}"
...
def build_month_notebook(root: Path, ym: str) -> Path:
    ...
    for e in entries:
        cells.append(new_markdown_cell(entry_cell_markdown(e)))

    entries_literal = pprint.pformat(entries, width=100, sort_dicts=False)
    cells.append(new_code_cell(BOOTSTRAP))
    cells.append(new_code_cell(f"entries = {entries_literal}\n"))
    cells.append(new_code_cell(
        "print(journal.summary_line(entries))\n"
    ))
    cells.append(new_code_cell(
        f'journal.plot_r_bars(entries, "R per trade — {label}")\n'
    ))
    cells.append(new_code_cell(
        f'journal.plot_equity_curve(entries, "Cumulative R — {label}")\n'
    ))
    cells.append(new_markdown_cell(
        "*Price charts below use Yahoo Finance and need a normal internet "
        "connection — they render silently if that's not available.*"
    ))
    cells.append(new_code_cell(
    ...
    out_path = root / "notebooks" / f"{ym}.ipynb"
...
def build_month_markdown(root: Path, ym: str) -> Path:
    ...
    lines = [f"# {label}", ""]
    for e in entries:
        lines.append(entry_cell_markdown(e))
        lines.append("")
    lines.append("---")
    lines.append("")
    lines.append(f"*{_summary_from_entries(entries)}*")
    out_path = root / "trades" / f"{ym}.md"
...
def build_rollup_notebook(root: Path, months: list[str], title: str, out_path: Path,
    ...
    links_md = "\n".join(f"- [{r['month']}]({month_link_prefix}/{r['ym']}.ipynb)" for r in month_rows)
...
def build_year_notebook(root: Path, year: str, months: list[str]) -> Path:
    return build_rollup_notebook(
        ...
        root / "notebooks" / "years" / f"{year}.ipynb",
        month_link_prefix="..",
    )
...
def build_global_notebook(root: Path) -> Path:
    ...
    return build_rollup_notebook(
        ...
        root / "notebooks" / "global_track_record.ipynb",
        month_link_prefix=".",
...
def main():
    ...
    print("\n== Month notebooks + markdown ==")
    for ym in months:
        if ym not in MONTHS:
            continue
        built.append(build_month_notebook(root, ym))
    ...
    print(f"\nDone: {len(built)} notebook(s) generated.")
